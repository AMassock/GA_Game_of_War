...
function reShuffle(playerOne, playerTwo) {
    // let reshuffleOne = shuffleDeck(playerOne.discard);
    // let reshuffleTwo = shuffleDeck(playerTwo.discard);
    shuffleDeck(playerOne.discard);
    shuffleDeck(playerTwo.discard);

    for(let i = 0; playerOne.discard.length > 0; i++) {
        playerOne.deck.push(playerOne.discard[i]);
        playerOne.discard.pop();
    }

    for(let i = 0; playerTwo.discard.length > 0; i++) {
...
while(playerOne.deck != 0 || playerTwo.deck != 0) {
    ...
    if((playerOne.deck.length == 0 && playerTwo.deck.length == 0) && (playerOne.discard.length > 0 && playerTwo.discard.length > 0)) {
        ...
        for(let i = 0; ((playerOne.discard.length > 0) && (playerOne.discard.length > 0)); i++) {
